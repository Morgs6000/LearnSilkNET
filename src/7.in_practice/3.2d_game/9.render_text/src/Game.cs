...
public class Game : IDisposable
{
    ...
    public void Init()
    {
        ...
        Text = new TextRenderer(_gl, Width, Height);
    ...
    public void ProceessInput(float dt)
    {
        ...
        {
            if (Keys[(int)Key.Enter] && !KeysProcessed[(int)Key.Enter])
            {
                State = GameState.GAME_ACTIVE;

                KeysProcessed[(int)Key.Enter] = true;
            }
            if (Keys[(int)Key.W] && !KeysProcessed[(int)Key.W])
            {
                Level = (Level + 1) % 4;
                
                KeysProcessed[(int)Key.W] = true;
            }
            if (Keys[(int)Key.S] && !KeysProcessed[(int)Key.S])
            {
                if (Level > 0)
                {
                    --Level;
                ...
                KeysProcessed[(int)Key.S] = true;
            }
        }
        if (State == GameState.GAME_WIN)
        {
            if (Keys[(int)Key.Enter])
            {
                KeysProcessed[(int)Key.Enter] = true;
