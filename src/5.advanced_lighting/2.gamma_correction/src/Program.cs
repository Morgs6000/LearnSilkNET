...
public class Program
{
    ...
    private static uint _floorTextureGammaCorrected;

    // informações de iluminação
    // --------------------------------------------------
    private static Vector3[] _lightPositions =
    {
    ...
    private static Vector3[] _lightColors =
    {
        new Vector3(0.25f),
        new Vector3(0.50f),
        new Vector3(0.75f),
    ...
    private static void OnLoad()
    {
        ...
        // --------------------------------------------------
        _shader = new Shader(_gl, "src/gamma_correction.vs", "src/gamma_correction.fs");
    ...
    private static void OnRender(double deltaTime)
    {
        ...
        _gl.Uniform3(_gl.GetUniformLocation(_shader.ID, "lightPositions"), 4, _lightPositions.SelectMany(v => new[] { v.X, v.Y, v.Z }).ToArray());
        _gl.Uniform3(_gl.GetUniformLocation(_shader.ID, "lightColors"), 4, _lightColors.SelectMany(v => new [] { v.X, v.Y, v.Z }).ToArray());
        ...
        _gl.BindTexture(TextureTarget.Texture2D, _gammaEnabled ? _floorTextureGammaCorrected : _floorTexture);
